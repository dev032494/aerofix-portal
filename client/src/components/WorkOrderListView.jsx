...
export default function WorkOrderListView({ activeUser }) {
  ...
  return (
    <div className="h-full flex flex-col gap-4">
      {/* HEADER SECTION */}
      <div className="flex justify-between items-center bg-slate-900 p-5 rounded-2xl border border-slate-800 shadow-sm shrink-0">
        <div className="flex items-center gap-3">
          <div className="p-2 bg-sky-500/10 rounded-lg">
            <ClipboardList className="h-6 w-6 text-sky-400" />
          </div>
          <div>
            <h2 className="text-xl font-bold text-white">Work Order List</h2>
            <p className="text-xs text-slate-400 mt-0.5">Manage and track active maintenance orders</p>
          </div>
        </div>

        <button
          onClick={() => openModal()}
          className="bg-sky-600 hover:bg-sky-500 text-white px-4 py-2.5 rounded-xl font-semibold flex items-center gap-2 transition-all shadow-lg shadow-sky-900/20 active:scale-95"
        >
            ...
            <form onSubmit={handleSubmit} className="p-6 flex flex-col gap-5">
              ...
              <div className="flex justify-end gap-3 mt-2">
                <button
                  ...
                  className="px-5 py-2.5 rounded-xl font-semibold text-slate-300 bg-slate-800 hover:bg-slate-700 transition-colors"
                >
                  Cancel
                </button>
                <button
                  type="submit"
                  className="px-5 py-2.5 rounded-xl font-semibold text-white bg-sky-600 hover:bg-sky-500 shadow-lg shadow-sky-900/20 transition-all active:scale-95"
                >
