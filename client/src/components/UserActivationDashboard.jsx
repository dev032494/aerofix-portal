  ...
  return (
    <div className="w-full h-full flex flex-col">
      <div className="w-full bg-slate-950 border border-slate-850 rounded-xl sm:rounded-2xl shadow-2xl overflow-hidden flex flex-col flex-1 max-h-[calc(100vh-2rem)]">
        ...
        {/* Header Section */}
        <div className="px-4 py-4 sm:px-6 sm:py-5 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-b border-slate-800 bg-slate-900/95 shrink-0">
          <div className="flex items-start gap-3 min-w-0">
            <div className="p-2.5 bg-amber-500/10 text-amber-400 border border-amber-500/20 rounded-xl shrink-0 mt-1">
              <ShieldAlert className="h-6 w-6" />
            </div>
            <div className="min-w-0">
              <h1 className="text-xl md:text-2xl font-black tracking-wider text-white uppercase truncate">User Activation Logs</h1>
              <p className="text-xs md:text-sm text-slate-400 mt-0.5 truncate">Review system validation histories and modify access privileges.</p>
            </div>
          </div>
          <button
            onClick={openModal}
            className="w-full sm:w-auto px-4 py-2.5 bg-amber-600 hover:bg-amber-500 text-slate-950 text-xs md:text-sm font-black rounded-xl shadow-lg shadow-amber-600/10 transition-all uppercase cursor-pointer shrink-0"
          >
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left border-collapse min-w-[700px]">
                ...
                <tbody className="text-xs divide-y divide-slate-800/60 bg-slate-900/20">
                  {loading ? (
                    <tr>
                      <td colSpan="4" className="p-12 text-center text-sky-400 font-medium animate-pulse">
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-xl shadow-2xl w-full max-w-md overflow-hidden">
            ...
            <form onSubmit={handleFormSubmit} className="p-5 space-y-4">
              ...
              <div className="flex justify-end gap-3 pt-4 border-t border-slate-800">
                <button type="button" onClick={closeModal} disabled={submitting} className="px-4 py-2 bg-slate-950 text-slate-400 text-xs font-bold uppercase border border-slate-800 rounded-xl cursor-pointer">
                  Close
                </button>
                <button type="submit" disabled={submitting} className="px-4 py-2 bg-amber-600 text-slate-950 text-xs font-black uppercase rounded-xl cursor-pointer">
